...
const MOCK_SHOPS: Record<string, ShopProfile> = {
  "3": {
    ...
    players: [
      { id: "1", nickname: "糖糖不甜", avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=sugar", gameCategories: ["王者荣耀", "英雄联盟"], pricePerHour: 35, rating: 4.9, online: true },
      { id: "7", nickname: "小鱼干", avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=fish", gameCategories: ["和平精英"], pricePerHour: 28, rating: 4.7, online: true },
      { id: "8", nickname: "星辰大海", avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=ocean", gameCategories: ["原神"], pricePerHour: 32, rating: 4.8, online: false },
      { id: "9", nickname: "闪电侠", avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=flash", gameCategories: ["英雄联盟", "CS2"], pricePerHour: 45, rating: 4.9, online: true },
    ],
  },
  "6": {
    id: "6",
    ...
    players: [
      { id: "10", nickname: "月牙儿", avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=crescent", gameCategories: ["王者荣耀"], pricePerHour: 40, rating: 4.6, online: true },
      { id: "11", nickname: "夜精灵", avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=elf", gameCategories: ["英雄联盟"], pricePerHour: 42, rating: 4.5, online: false },
      { id: "12", nickname: "星河漫步", avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=galaxy", gameCategories: ["原神", "崩坏：星穹铁道"], pricePerHour: 38, rating: 4.7, online: true },
    ],
  },
};
        ...
        <div className="absolute -bottom-12 left-1/2 -translate-x-1/2 z-10">
          <div className="relative">
            <img
              src={shop.avatar}
              alt={shop.shopName}
              className="w-24 h-24 rounded-2xl border-4 border-[#0f0f1a] object-cover"
            />
            {shop.online && (
            ...
            <div className="flex items-center justify-center gap-2 mb-1">
              ...
              {shop.online ? (
                <span className="flex items-center gap-1 text-sm text-green-400">
                  <span className="w-2 h-2 bg-green-500 rounded-full" /> 营业中
                </span>
              ) : (
                <span className="flex items-center gap-1 text-sm text-gray-500">
                  <span className="w-2 h-2 bg-gray-500 rounded-full" /> 休息中
                </span>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {shop.players.map((p) => (
                <Link
                  key={p.id}
                  href={`/profile/${p.id}`}
                  className="glass rounded-xl p-4 hover:bg-white/10 transition flex items-center gap-4"
                >
                  <div className="relative shrink-0">
                    <img src={p.avatar} alt={p.nickname} className="w-14 h-14 rounded-xl object-cover" />
                    {p.online && (
                      <span className="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 bg-green-500 rounded-full border-2 border-[#1a1a2e]" />
                    )}
                  </div>
